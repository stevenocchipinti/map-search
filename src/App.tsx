...
function App() {
  ...
  useEffect(() => {
    if (searchResults) {
      ...
      setSearchResults(prev =>
        prev ? { ...prev, schools: filteredSchools } : null
      )
      ...
      if (filteredSchools.length > 0) {
        ...
        if (!existingRoute) {
          ...
          fetchRoutesSequentially([{ from: location, to: topSchool }])
            ...
            .catch(err => {
              console.error(
                "Failed to fetch school route after filter change:",
                err
              )
              setRouteLoadingStates(prev => ({ ...prev, school: false }))
            })
  ...
  /**
   * Handle map click by immediately running the same coordinate search flow.
   */
  const handleMapClick = async (lat: number, lng: number): Promise<void> => {
    ...
    await handleSearchWithCoordinates(lat, lng, state, "Pinned Location")
  }
  ...
  const handleSelectPOI = (category: POICategory, index: number): void => {
    ...
    if (searchResults) {
      ...
      if (!route) {
        ...
        fetchRoutesSequentially([{ from: searchResults.location, to: poi }])
          .then(() => {
            setRouteLoadingStates(prev => ({ ...prev, [category]: false }))
          })
          .catch(err => {
  ...
  useEffect(() => {
    ...
    const selectedPOIsArray = [
      ...(searchResults.schools[selectedPOIs.school]
        ? [searchResults.schools[selectedPOIs.school]]
        : []),
      ...(searchResults.stations[selectedPOIs.station]
        ? [searchResults.stations[selectedPOIs.station]]
        : []),
      ...(searchResults.supermarkets[selectedPOIs.supermarket]
        ? [searchResults.supermarkets[selectedPOIs.supermarket]]
        : []),
    ]
  ...
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      {updateAvailable && (
        <div className="bg-blue-600 text-white px-5 py-3.5 flex items-center justify-between shadow-soft-md z-50 dark:bg-blue-700">
      ...
      <div className="flex md:hidden flex-col h-full relative">
        ...
        {!isOnline && !offlineBannerDismissed && (
          <div className="absolute top-0 left-0 right-0 z-[1001] bg-amber-50 border-b border-amber-200 px-4 py-2 flex items-center justify-between dark:bg-amber-950 dark:border-amber-800">
            ...
            <button
              ...
              className="text-amber-600 hover:text-amber-800 transition-colors duration-200 dark:text-amber-400 dark:hover:text-amber-200"
        ...
        {showLanding ? (
          <AttributionToggle hasSearched={false} isLanding={true} />
        ) : (
          <AttributionToggle
            hasSearched={hasSearched || !!userLocation}
            isLanding={false}
