...
export function useSnapPoints({
    ...
    return (
      snapPoints?.map((snapPoint) => {
        ...
        if (isVertical(direction)) {
          const height = isPx ? snapPointAsNumber : windowDimensions ? snapPoint as number * containerSize.height : 0;
        ...
        const width = isPx ? snapPointAsNumber : windowDimensions ? snapPoint as number * containerSize.width : 0;
  ...
  const snapToPoint = React.useCallback(
    ...
    [drawerRef, snapPoints, snapPointsOffset, fadeFromIndex, overlayRef, setActiveSnapPoint, direction, onSnapPointChange],
  );
