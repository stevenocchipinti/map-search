...
interface WalkingRoutesResult {
  ...
  fetchRoutesSequentially: (requests: Array<{ from: SearchLocation; to: POI }>) => Promise<void>;
...
export function useWalkingRoutes(): WalkingRoutesResult {
  ...
  const getCachedRoute = useCallback((from: SearchLocation, to: POI): WalkingRoute | undefined => {
    const key = getCacheKey(from, to);
    return routeCache.get(key);
  }, [routeCache]);

  const isLoading = useCallback((from: SearchLocation, to: POI): boolean => {
    const key = getCacheKey(from, to);
  ...
  const fetchRoute = useCallback(async (
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to fetch route';
  ...
  const fetchRoutesSequentially = useCallback(async (
    requests: Array<{ from: SearchLocation; to: POI }>
  ): Promise<void> => {
    console.log(`Fetching ${requests.length} routes sequentially with ${FETCH_DELAY_MS}ms delays`);
