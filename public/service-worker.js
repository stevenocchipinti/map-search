      ...
      .then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cacheName) => {
            ...
            if (cacheName.startsWith('map-search-') && cacheName !== CACHE_NAME && 
                cacheName !== DATA_CACHE_NAME && cacheName !== API_CACHE_NAME &&
...
self.addEventListener('fetch', (event) => {
  ...
  if (DATA_FILES_PATTERN.test(url.pathname)) {
    ...
    // API calls: Network-first with cache fallback
    event.respondWith(networkFirstStrategy(request, API_CACHE_NAME, MAX_API_CACHE_SIZE));
  } else if (TILES_PATTERN.test(url.href)) {
    // Map tiles: Cache-first with expiration
    event.respondWith(cacheFirstStrategy(request, TILES_CACHE_NAME, MAX_TILES_CACHE_SIZE));
...
async function manageCacheSize(cacheName, maxSize) {
  ...
  if (keys.length > maxSize) {
    console.log(`[SW] Cache ${cacheName} exceeds ${maxSize} entries, removing oldest`);
