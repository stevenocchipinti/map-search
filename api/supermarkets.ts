...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (latitude < -90 || latitude > -10) {
      return res
        .status(400)
        .json({ error: "Latitude out of range for Australia" })
    }

    if (longitude < 110 || longitude > 155) {
      return res
        .status(400)
        .json({ error: "Longitude out of range for Australia" })
    }

    // Fetch supermarkets from Overpass
    const supermarkets = await findNearbySupermarkets(
      latitude,
      longitude,
      searchRadius
    )
    ...
    const pois: SupermarketPOI[] = supermarkets.map(supermarket => {
      ...
      return {
        ...
        details:
          [supermarket.suburb, supermarket.postcode]
            .filter(Boolean)
            .join(", ") || undefined,
      }
    })
